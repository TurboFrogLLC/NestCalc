...
| **Two-box ticker** | Travel box + picker box with **8px air** and flush right edges. Cut A: hits/inputs **28**, pad **6 T/B · 8 L/R**, item gap **2**, hit/input/shell radius **8**, outer **42.2**, travel icon **18**. Calc lives in travel. Door chevron last. Edit replaces travel. Picker chips are words only (Part / Gap / Margin / Reset). Send (`square-arrow-out-up-right`) opens the HUD Bed Presets face for Blank/Gap/Margin. Not `.param-popover` WRITE. Gap/XY first paint: X ≠ Y → link off. Ticker/picker do not call `__howManyOpenField`. | `#bt-travel-box` · `#bt-picker` · `#bt-edit` · `#preset-door` |
...
| HUD presets (`#preset-door`) | HUD shell, expand behavior, and card size stay. The numeric pad is replaced by a Bed Presets face: **Blank**, **Gap**, and **Margin** rows only; each has slots **1–5** and **+**. No Part row or material tree. Selecting a slot lights it and loads it into the in-panel ticker; typing updates that slot only and does not affect the laser bed. `+` selects the first empty slot (or 5 if full). **Send to bed** saves the lit slot then hydrates Blank, Gap, or all four Margin values onto the laser ticker and bed, whether the picker is open or shut. Travel calc opens this face on Blank when no ticker field is in edit. Storage remains `howmany.flipit.v3.presets`. |
