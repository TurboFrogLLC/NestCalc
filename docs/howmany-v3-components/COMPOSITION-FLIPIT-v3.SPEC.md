...
**Status:** Living (locked menu-bar ticker — three-decimal ticker inputs)
**Product:** **FlipIt**  
...
| **Menu-bar ticker** | Picker sits **above** and attached to travel, right-flush, never full-width. Picker height **22**, word gap **8**, picker pad **0 T/B · 8 L/R**, picker outer radius **6**; no 8px air. Closed, only the ticker has a bounding stroke. Open, its stroke becomes an inverted-L: ticker top-left radius **8** and top stroke stay visible left of the picker; the travel outer top-right stays flat while a **convex 6px grid quarter** walks the 1.1px outline around the inside left step. It is filled `#E8E8E8`; the picker-left × travel-top join has no concave scoop. The picker keeps left/top/right strokes, overlaps the travel top by **1.1px**, and has no bottom border. An inset-only **1.1px** `rgba(26, 20, 40, 0.22)` join rule returns under the picker with **8px** clearance at each side. Picker words only: Part / Gap / Margin / Reset; no boxed controls. Travel stays outer **42.2** with hits/inputs **28**, pad **6 T/B · 8 L/R**, item gap **2** except the paired −90/+90 hits are flush; travel icon **18**. Calc stays in travel; door chevron last; edit replaces travel. Ticker and Bed Presets edit inputs are radius **6**. Blank, Part, and Gap edit X/Y inputs sit adjacent with Link only; there is no swap control. Ticker Send safely exits ticker edit after opening the Bed Presets face, leaving ticker, picker, bed, HUD, and zoom interactive. Ticker stays static while the picker clips and slides up/down. Ticker hits, readout, picker words, calc, chevron, and rotate stay visually idle after tap; only the linked X/Y control may use `is-on`. Their tap highlights are transparent and click blurs focus. Bed ticker and Bed Presets face inputs cap at **3 decimal places** through the same HUD popover sanitizer. A cleared ticker or preset-face field is visibly rejected and focused on either Send action; no hidden prior draft may be sent. Send (`square-arrow-out-up-right`) writes the ticker draft to the HUD Bed Presets row for Blank/Gap/Margin: first empty unless a slot was explicitly lit during this visit. Not `.param-popover` WRITE. Gap/XY first paint: X ≠ Y → link off. Ticker/picker do not call `__howManyOpenField`. | `#bt-travel-box` · `#bt-picker` · `#bt-edit` · `#preset-door` |
...
| Blank ticker menu-bar (`#blank-ticker-cluster`) | Travel box: `[ −90 ][ +90 ]  readout  [calc] [▾]`. Outer **42.2** (28 + 6 + 6 + 1.1 + 1.1). Hits and edit inputs **28**. Pad **6 T/B · 8 L/R**. Item gap **2**, except the adjacent −90/+90 hits have no gap. Open picker is attached **above** travel, right-flush, height **22**, word gap **8**, pad **0 T/B · 8 L/R**, and outer radius **6**. Closed, the ticker is one rounded bounding stroke. Open, the travel top-right stays flat and the picker left × travel-top connector is a 6px concave fillet; the picker overlaps its top by **1.1px**. The picker retains left/top/right strokes and no bottom border; its 1.1px join rule is inset **8px** at each side, preserving gaps from picker left and right. Picker controls are plain words only (Part / Gap / Margin / Reset); Reset stays a word. The ticker remains static while the picker clips and slides up/down. Ticker hits, readout, picker words, calc, chevron, and rotate stay idle after tap: hover/focus fills are absent, tap highlight is transparent, and click blurs focus. Only the linked X/Y control may show `is-on`. Ticker and Bed Presets inputs use radius 6. Blank, Part, and Gap show adjacent X/Y input pairs and Link only; Margin has no swap. Click a field word (or the readout) turns the travel box into the field — axis letters on the bar (Part/Gap **X Y**, Margin **L R B T**); lit word is the label. Focus select-all on edit inputs. The bed ticker and the Bed Presets face both cap input at **3 decimal places** via the HUD popover sanitizer. OK commits, Cancel drops the draft. Send (`square-arrow-out-up-right`) saves the current ticker draft into the first empty slot, opens that Bed Presets face, and exits ticker edit without leaving an interaction-blocking overlay. It does not apply the draft to the bed or open the floating WRITE card. Gap link first paint: X ≠ Y → link off. Reset runs on the picker. Ticker `dblclick` does not open HUD. No `__howManyOpenField` from ticker or picker. |
...
| Zoom chrome | `#lb-zoom-in` / `#lb-zoom-out` / `#lb-fit` z **82** — shares the front surface with blank ticker + calc and Numeric HUD. Default, Fit, viewport resize, and phone rotation center the blank with 2in air on every side through the same blank-fit path. Wheel zoom and canvas drag pan remain available. Blank-handle drags resize only, and resizing the blank does not live re-fit the camera. |
...
| HUD viewport clamp | Opening calculator or Bed Presets keeps `#hud` inside the visible webview. On a phone with FLiPIT closed, it is unscaled, clamps above the ticker when space permits, and layers above the ticker instead of retaining the initial `25,25` placement. Closing FLiPIT after its 75% composition scale, plus viewport resize or phone rotation while calculator or Bed Presets is open, re-runs this unscaled clamp. The 75% HUD scale is reserved for the open FLiPIT composition. |
...
5. Blank ticker is a locked menu-bar. Travel holds ±90, readout, calc, chevron-last. Outer 42.2, hits/inputs 28, pad 6/8, and item gap 2 remain. Open the 22px picker: Part / Gap / Margin / Reset are plain words with 8px gaps, directly attached above travel and right-flush, with 0 T/B and 8 L/R padding plus radius 6. The ticker stays static while the picker slides; its top-left remains radius 8 and only the shared top-right becomes square. There is no line beneath the picker: the joined pair is one shell. Field words edit in the travel box; they do not pour HUD. Ticker controls and picker words stay idle after tap, with transparent tap highlight and click blur; only linked X/Y uses `is-on`. Send morphs `#hud` calculator presets, not the WRITE popover. Gap X ≠ Y paints link off. AUTO-SIZE does **not** open FLiPIT. `#btn-detect` still sizes. HUD FLiPIT cycle from R3 is unchanged. HowMany `#lb-count` shows the live `calculateNest` total past `#lb-hit-corner` on the 45° ray.
...
| 2026-08-31 | **Ticker menu-bar** Locked desktop chrome: picker moved above and attached to the ticker, right-flush, 22px high, 8px word gap, zero pad and zero-radius join. Part / Gap / Margin / Reset remain words; Reset is not a button. Travel retains ±90, calc, and in-ticker editing. |
| 2026-08-31 | **Ticker menu-bar Cut B look** Picker pad is 0 T/B · 8 L/R with outer radius 6. The static ticker keeps top-left radius 8; only its shared top-right squares beneath the picker. Picker clips and slides without moving cluster top; ticker controls and picker words have no press imprint. |
| 2026-08-31 | **Ticker menu-bar Cut C look** The picker masks the travel top-border beneath its overlap, leaving no join line. Cluster hover/focus fills are removed; only linked X/Y may retain `is-on`. Ticker hits and words use transparent tap highlight and blur after click. |
| 2026-08-31 | **Ticker menu-bar Cut D shell** Closed ticker retains the only bounding stroke. Open picker overlaps the travel top stroke by 1.1px, keeps left/top/right strokes, and has no bottom stroke, forming an inverted-L without a full-width empty shell. |
| 2026-08-31 | **Ticker menu-bar Cut E look** The picker join rule returns only between 8px side insets; its L/T/R shell and 1.1px overlap remain. The travel’s inside top-right corner is radius 6, and the −90/+90 hits are flush while the other travel gaps remain 2px. |
| 2026-08-31 | **Ticker menu-bar Cut F look** The travel outer right join is flat; the 6px inverse fillet moves to the picker-left connector. Ticker and Bed Presets inputs are radius 6. Ticker X/Y edit loses swap, and Send exits edit safely after opening the selected Bed Presets face. |
| 2026-08-31 | **Ticker menu-bar Cut G look** The Cut F inverse scoop is removed. A filled 6px convex quarter in the grid walks the 1.1px outline around the picker-left × travel-top step; shared edges stay square, the travel outer right stays flat, and the 8px inset join rule remains. |
