...
| **Two-box ticker** | Travel box + picker box with **8px air** and flush right edges. Cut A: hits/inputs **28**, pad **6 T/B · 8 L/R**, item gap **2**, hit/input/shell radius **8**, outer **42.2**, travel icon **18**. Calc lives in travel. Door chevron last. Edit replaces travel. Picker chips are words only (Part / Gap / Margin / Reset). Send (`square-arrow-out-up-right`) morphs `#hud` calculator preset surface for Blank/Gap/Margin. Not `.param-popover` WRITE. Gap/XY first paint: X ≠ Y → link off. Ticker/picker do not call `__howManyOpenField`. | `#bt-travel-box` · `#bt-picker` · `#bt-edit` · `__hudPresetMorph` |
