...
| **C1 ticker door** | The blank ticker expands to one action strip: Part / Blank / Gap / Margin chips, Part 90°, and Reset. A loaded program adds AUTO-SIZE / FLiPIT to that strip only. | `#blank-ticker` → `#ticker-door` · field chips → existing HUD popovers · Reset → `clearManualInputs` · Part 90° → `rotateManualPart` |
...
| Blank ticker EXPAND (`#blank-ticker`) | Collapsed stays a blank-size readout. Expand opens exactly one ticker-door strip with Part / Blank / Gap / Margin chips, **Part 90°**, and **Reset**. Field chips use the existing HUD popovers; Part 90° follows `rotateManualPart`; Reset follows `clearManualInputs` and works without a loaded file. Once source is loaded, that same strip adds **AUTO-SIZE** and **FLiPIT**; AUTO-SIZE still opens only the native picker and FLiPIT stays an explicit separate control. |
...
5. Blank ticker stays a readout until expanded. Its one door strip exposes Part / Blank / Gap / Margin, Part 90°, and Reset; after a program loads it also exposes AUTO-SIZE / FLiPIT. AUTO-SIZE does **not** open FLiPIT. `#btn-detect` still sizes. HUD FLiPIT cycle from R3 is unchanged. HowMany `#lb-count` shows the live `calculateNest` total past `#lb-hit-corner` on the 45° ray.
