...
**Status:** Living (two-box ticker — travel + picker, 8px air, travel vs edit)
**Product:** **FlipIt**  
...
| **Two-box ticker** | Travel box + picker box with **8px air**. Calc lives in travel. Door chevron last (`chevron-down` / `chevron-up`). Edit replaces travel (±90 and calc leave). Picker may stay. Send/bookmark morphs `#hud` presets only. Ticker/picker do not call `__howManyOpenField`. | `#bt-travel-box` · `#bt-picker` · `#bt-edit` · `__hudPresetMorph` |
...
| Blank ticker two-box (`#blank-ticker-cluster`) | Travel box: `[ −90 ][ +90 ]  readout  [calc] [▾]`. Picker open slides travel up by picker height + **8px air**; blank and bed stay put. Picker: Part / Gap / Margin / Reset. Click a field chip (or the readout) turns the travel box into the field — no field word on the bar; lit picker chip is the label. OK commits, Cancel drops the draft, Send/bookmark earns `#hud` preset morph only. Reset runs on the picker. Ticker `dblclick` does not open HUD. No `__howManyOpenField` from ticker or picker. |
...
5. Blank ticker is two boxes. Travel holds ±90, readout, calc, chevron-last. Picker (Part / Gap / Margin / Reset) sits 8px under travel and may stay while editing. Field chips edit in the travel box; they do not pour HUD. Send/bookmark morphs HUD presets only. AUTO-SIZE does **not** open FLiPIT. `#btn-detect` still sizes. HUD FLiPIT cycle from R3 is unchanged. HowMany `#lb-count` shows the live `calculateNest` total past `#lb-hit-corner` on the 45° ray.
