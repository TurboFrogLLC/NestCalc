...
**Status:** Living (two-box ticker — look-fit 28 hit / 36.2 outer, icon-left picker words)
**Product:** **FlipIt**  
...
| **Two-box ticker** | Travel box + picker box with **8px air**. Look-fit: hits/inputs **28**, pad **3 T/B · 8 L/R**, radius **10**, outer **36.2**, travel icon **18**, picker icon **16**. Calc lives in travel. Door chevron last. Edit replaces travel. Picker chips are icon-left + word (`box` Part, `between-horizontal-start` Gap, `align-start-vertical` Margin, `eraser` Reset). Send (`square-arrow-out-up-right`) morphs `#hud` calculator preset surface for Blank/Gap/Margin. Not `.param-popover` WRITE. Gap/XY first paint: X ≠ Y → link off. Ticker/picker do not call `__howManyOpenField`. | `#bt-travel-box` · `#bt-picker` · `#bt-edit` · `__hudPresetMorph` |
...
| Blank ticker two-box (`#blank-ticker-cluster`) | Travel box: `[ −90 ][ +90 ]  readout  [calc] [▾]`. Outer **36.2** (28 + 3 + 3 + 1.1 + 1.1). Hits and edit inputs **28**. Pad **3 T/B · 8 L/R**. Radius **10**. Picker open slides travel up by picker height + **8px air**. Picker chips: icon left + word (Part / Gap / Margin / Reset). Click a field chip (or the readout) turns the travel box into the field — axis letters on the bar (Part/Gap **X Y**, Margin **L R B T**); lit picker chip is the label. Focus select-all on edit inputs. OK commits, Cancel drops the draft. Send (`square-arrow-out-up-right`) morphs `#hud` calculator presets for Blank/Gap/Margin after exiting calc; does not open the floating WRITE card. Gap link first paint: X ≠ Y → link off. Reset runs on the picker. Ticker `dblclick` does not open HUD. No `__howManyOpenField` from ticker or picker. |
...
| LaserBed calc chip (`#bt-calc`) | HUD collapsed → expand to **param** mode only (do not enter calculator; clear leftover classic-calc `display`). HUD already expanded → toggle calculator. After exit / collapse / expand, `#hud-body` height matches the active mode. Hit square **28** inside the **36.2** travel box. Travel icon **18**. 3px gap. Hover **lightens the dark fill only** — icon and chrome stay visible (no transparent wash). |
