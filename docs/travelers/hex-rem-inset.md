...
Arms
- hex off + hamburger on = rectangle AutoNest
- hex on + skull off = virgin hex. No cutouts. Ignore hole D.
- hex on + skull on = skeleton.
Hex on replaces hamburger with Lucide skull (viewBox 24):
...
Tooltip: skeleton cut. Armed color same as hex (#16A34A).

Init card (skull click, same chrome as preset card)
...
Ticket: COLUMNS, ROWS, X GAP, Y GAP, X ORIGIN (that leftmost full-row X from rem 0,0), POCKET COLS (example 4-6).
...
Cutouts red. Not in blue count. Skull off = virgin hex. No second cutout row. No NC.
